...
def Home(root, frame1, frame2):
    ...
    Button(frame2, text="Home", bg='#648fde', command = lambda: Home(root, frame1, frame2)).grid(row=0,column=0)
    ...
    #Admin Login
    admin = Button(frame1, text="Admin Login", width=15, bg='#648fde', command = lambda: AdmLogin(root, frame1))

    #Voter Login
    voter = Button(frame1, text="Voter Login", width=15, bg='#648fde', command = lambda: voterLogin(root, frame1))

    #New Tab
    newTab = Button(frame1, text="New Window", width=15, bg='#648fde', command = lambda: sb_p.call('start python homePage.py', shell=True))
